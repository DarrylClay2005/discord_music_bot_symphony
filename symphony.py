                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'symphony', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
                                elif loop_mode == 'song':
                                    await cur.execute("SELECT COALESCE(MIN(id), 0) FROM symphony_queue WHERE guild_id = %s", (payload.player.guild.id,))
                                    min_row = await cur.fetchone()
                                    new_id = ((min_row[0] if min_row else 0) or 0) - 1
                                    await cur.execute("INSERT INTO symphony_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'symphony', %s, %s, %s)", (new_id, payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
        except Exception:
            pass
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            await cur.execute("SELECT MIN(id) as m_id FROM symphony_queue WHERE guild_id = %s", (guild.id,))
                            min_res = await cur.fetchone()
                            new_id = (min_res['m_id'] or 0) - 1
                            await cur.execute("INSERT INTO symphony_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'symphony', %s, %s, %s)", (new_id, guild.id, orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id))
...
async def queue_cmd(interaction: discord.Interaction):
    ...
    else: await interaction.response.send_message(embed=discord.Embed(description="Queue empty.", color=discord.Color.red()), ephemeral=True)
...
async def grab(interaction: discord.Interaction):
    if interaction.guild.id in playback_tracking:
        ...
        dm_embed = discord.Embed(title="🎵 Track Saved!", description=f"Hey **{interaction.user.display_name}**!\nHere is the track you wanted to save:\n\n**[{data.get('title', 'Unknown Title')}]({data['url']})**", color=discord.Color.from_rgb(88, 101, 242))
        try:
        ...
        except discord.Forbidden: await interaction.response.send_message(embed=discord.Embed(description="❌ I can't DM you! Please check your privacy settings.", color=discord.Color.red()), ephemeral=True)
    else: await interaction.response.send_message(embed=discord.Embed(description="Nothing is currently playing.", color=discord.Color.red()), ephemeral=True)
...
async def volume(interaction: discord.Interaction, vol: int):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute("INSERT INTO symphony_guild_settings (guild_id, volume) VALUES (%s, %s) ON DUPLICATE KEY UPDATE volume = %s", (interaction.guild.id, vol, vol))
    if interaction.guild.voice_client:
...
async def loop_cmd(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute("INSERT INTO symphony_guild_settings (guild_id, loop_mode) VALUES (%s, %s) ON DUPLICATE KEY UPDATE loop_mode = %s", (interaction.guild.id, mode, mode))
...
@bot.tree.command(name="symphony_main_filter", description="Apply an audio filter")
@app_commands.choices(mode=[app_commands.Choice(name="None", value="none"), app_commands.Choice(name="Bassboost", value="bassboost"), app_commands.Choice(name="Nightcore", value="nightcore"), app_commands.Choice(name="Vaporwave", value="vaporwave")])
async def filter_cmd(interaction: discord.Interaction, mode: str):
    ...
    if interaction.guild.voice_client:
        f = wavelink.Filters()
        if mode == 'nightcore': f.timescale = wavelink.Timescale(speed=1.25, pitch=1.3)
        elif mode == 'vaporwave': f.timescale = wavelink.Timescale(speed=0.8, pitch=0.8)
        elif mode == 'bassboost': f.equalizer = wavelink.Equalizer(bands=[(0, 0.3), (1, 0.2), (2, 0.1)])
        try: await interaction.guild.voice_client.set_filters(f)
        except: pass
...
@bot.tree.command(name="symphony_main_modify", description="Change speed/pitch")
async def modify_audio(interaction: discord.Interaction, speed: float = 1.0, pitch: float = 1.0, duration: int = 1):
    if not await is_dj(interaction): return
    speed, pitch = max(0.5, min(2.0, speed)), max(0.5, min(2.0, pitch))
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
    ...
    if interaction.guild.voice_client:
        f = wavelink.Filters()
        f.timescale = wavelink.Timescale(speed=speed, pitch=pitch)
        try: await interaction.guild.voice_client.set_filters(f)
        except: pass
    await interaction.response.send_message(embed=discord.Embed(title="🎛️ Audio Modifiers Set", description=f"Speed: {speed}x, Pitch: {pitch}x", color=discord.Color.gold()), ephemeral=True)
...
async def seek(interaction: discord.Interaction, seconds: int):
    if not await is_dj(interaction) or interaction.guild.id not in playback_tracking: return
    if interaction.guild.voice_client:
        await interaction.guild.voice_client.seek(seconds * 1000)
        playback_tracking[interaction.guild.id].update({'offset': seconds, 'start_time': time.time()})
...
async def replay(interaction: discord.Interaction):
    ...
    if interaction.guild.voice_client:
        ...
        playback_tracking[interaction.guild.id].update({'offset': 0, 'start_time': time.time()})
...
# --- UTILITY & INFO ---
@bot.tree.command(name="symphony_main_panel", description="Music control panel")
async def panel(interaction: discord.Interaction):
    class P(discord.ui.View):
        @discord.ui.button(label="⏯️", style=discord.ButtonStyle.primary)
        async def pr(self, i, b):
            if not await is_dj(i) or not i.guild.voice_client: return
            p = i.guild.voice_client
            if p.playing: await p.pause(True)
            else: await p.pause(False)
            await i.response.send_message("Toggled Playback", ephemeral=True)
        ...
        async def sk(self, i, b):
            if not await is_dj(i) or not i.guild.voice_client: return
            await i.guild.voice_client.stop()
            await i.response.send_message("Skipped", ephemeral=True)
    await interaction.response.send_message(embed=discord.Embed(title="🎛️ Symphony Control Panel", color=discord.Color.from_rgb(43, 45, 49)), view=P())

@bot.tree.command(name="symphony_main_nowplaying", description="Show song status")
async def nowplaying(interaction: discord.Interaction):
    if interaction.guild.id in playback_tracking:
        d = playback_tracking[interaction.guild.id]
        cur = int((time.time() - d['start_time']) * d.get('speed', 1.0) + d['offset'])
        await interaction.response.send_message(embed=discord.Embed(title="🎵 Now Playing", description=f"**[{d['title']}]({d['url']})**\n`{make_progress_bar(cur, d['duration'])}`", color=discord.Color.blue()), ephemeral=True)
    else: await interaction.response.send_message(embed=discord.Embed(description="Nothing playing.", color=discord.Color.red()), ephemeral=True)

@bot.tree.command(name="symphony_main_help", description="Command List")
async def help_cmd(interaction: discord.Interaction):
    cmds = [c.name for c in bot.tree.get_commands() if c.name.startswith("symphony_main_")]
    await interaction.response.send_message(embed=discord.Embed(title="📚 Symphony Commands", description=", ".join(cmds), color=discord.Color.blue()), ephemeral=True)

@bot.tree.error
async def on_app_command_error(interaction, error):
    logger.error(f"Error: {error}")
    if not interaction.response.is_done(): await interaction.response.send_message(f"Error: `{error}`", ephemeral=True)

# --- LIVE PLAYLIST SYNC FEATURE ---
async def init_playlist_db():
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute('''CREATE TABLE IF NOT EXISTS symphony_active_playlists (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, known_track_count INT DEFAULT 0, requester_id BIGINT, PRIMARY KEY (guild_id, bot_name))''')
...
async def playlist_sync_loop():
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT guild_id, playlist_url, known_track_count, requester_id FROM symphony_active_playlists WHERE bot_name = 'symphony'")
    ...
    for gid, url, count, rid in pls:
        try:
            data = await loop.run_in_executor(None, lambda: ydl.extract_info(url, download=False))
            if not data or 'entries' not in data: continue
            entries = [e for e in data['entries'] if e]
            cur_count = len(entries)
            if cur_count > count:
                new_tracks = entries[count:]
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if u and not u.startswith('http'): u = f"https://www.youtube.com/watch?v={entry.get('id')}"
                                if u: await cur.execute("INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'symphony', %s, %s, %s)", (gid, u, t, rid))
                            await cur.execute("UPDATE symphony_active_playlists SET known_track_count = %s WHERE guild_id = %s", (cur_count, gid))
                guild = bot.get_guild(gid)
                if guild:
                    embed = discord.Embed(title="📡 Playlist Updated", description=f"Queued **{cur_count - count}** new tracks.", color=discord.Color.green())
                    await send_feedback(guild, embed)
                    await send_webhook_log(
                        bot.user.name if bot.user else "Unknown Node",
                        "📡 Playlist Sync",
                        f"Detected **{cur_count - count}** new tracks for guild {guild.name} and queued them automatically.",
                        discord.Color.green()
                    )
        except: pass
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT guild_id, command FROM symphony_swarm_overrides WHERE bot_name = 'symphony'")
                    commands = await cur.fetchall()
        if not commands: return
        for row in commands:
            guild = bot.get_guild(row['guild_id'])
            if guild and guild.voice_client:
                vc, cmd, exec = guild.voice_client, row['command'], False
                if cmd == 'PAUSE' and vc.playing: await vc.pause(True); exec = True
                elif cmd == 'RESUME' and vc.paused: await vc.pause(False); exec = True
                elif cmd in ['SKIP', 'STOP']: await vc.stop(); exec = True
                if cmd == 'STOP':
                    async with DBPoolManager() as pool:
                        async with pool.acquire() as conn:
                            async with conn.cursor() as cur: await cur.execute("DELETE FROM symphony_queue WHERE guild_id = %s AND bot_name = 'symphony'", (guild.id,))
                if exec: await send_webhook_log(bot.user.name, "🤖 Aria Override", f"Executed **{cmd}** in `{guild.name}`.", discord.Color.purple())
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur: await cur.execute("DELETE FROM symphony_swarm_overrides WHERE guild_id = %s AND bot_name = 'symphony'", (row['guild_id'],))
    except: pass
...
async def zombie_reaper_loop():
    for guild in bot.guilds:
        ...
        if vc and not vc.playing and not vc.paused:
            try:
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if cur.rowcount > 0: await send_webhook_log(bot.user.name, "🧹 Janitor", f"Cleared **{cur.rowcount}** records.", discord.Color.blurple())
    except: pass
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT * FROM symphony_swarm_direct_orders WHERE bot_name = 'symphony'")
                    orders = await cur.fetchall()
        if not orders: return
        for order in orders:
            guild = bot.get_guild(order['guild_id'])
            if guild:
                vc_target, cmd, data = order['vc_id'], order['command'], order['data']
                if cmd == 'PLAY' and vc_target:
                    await ensure_voice_connection(guild, vc_target)
                    try:
                        tracks = await wavelink.Playable.search(data)
                        if tracks:
                            es = tracks.tracks if isinstance(tracks, wavelink.Playlist) else [tracks[0] if isinstance(tracks, list) else tracks]
                            count = 0
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        for t in es:
                                            await cur.execute("INSERT INTO symphony_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'symphony', %s, %s, %s)", (guild.id, t.uri, t.title, bot.user.id))
                                            count += 1
                            if count > 0: await send_feedback(guild, discord.Embed(title="🎶 Order Received", description=f"Queued **{count}** tracks.", color=discord.Color.green()))
                    except: pass
                    if guild.voice_client and not guild.voice_client.playing: bot.loop.create_task(process_queue(guild, vc_target))
                elif cmd == 'LEAVE' and guild.voice_client:
                ...
                await send_webhook_log(bot.user.name, "🤖 Order Execution", f"Executed `{cmd}` order.", discord.Color.purple())
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur: await cur.execute("DELETE FROM symphony_swarm_direct_orders WHERE id = %s", (order['id'],))
    except: pass
...
class SwarmIntelligence(commands.Cog):
    def __init__(self, bot):
        self.bot, self.bot_name = bot, 'symphony'
                    ...
                    if guild.id in playback_tracking:
                        ...
                        if now - track_info.get('start_time', 0) > 10:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("SELECT id FROM symphony_queue WHERE guild_id = %s LIMIT 1", (guild.id,))
                                        ...
                                        if revival_attempts >= 3:
                                            playback_tracking.pop(guild.id, None)
                                            await cur.execute("UPDATE symphony_playback_state SET is_playing = FALSE WHERE guild_id = %s AND bot_name = 'symphony'", (guild.id,))
                                            await send_webhook_log(self.bot.user.name, "⚙️ Watchdog Cooldown", f"Stall persisted in `{guild.name}`; watchdog parked to prevent revival loop.", discord.Color.orange())
                                            continue
                                        track_info['watchdog_revival_attempts'] = revival_attempts + 1
                                        track_info['last_watchdog_revival'] = now
                                        track_info['start_time'] = now
                                        await send_webhook_log(self.bot.user.name, "⚙️ Watchdog Revival", f"Reviving stall in `{guild.name}`.", discord.Color.orange())
                                        self.bot.loop.create_task(process_queue(guild, track_info.get('channel_id')))
        except: pass
    @status_updater.before_loop
    @heartbeat.before_loop
    @watchdog.before_loop
    async def bl(self): await self.bot.wait_until_ready()

async def setup_intelligence(bot): await bot.add_cog(SwarmIntelligence(bot))
@bot.event
...
async def global_proximity_shield(interaction: discord.Interaction):
    ...
    if not interaction.command or not any(interaction.command.name.endswith(s) for s in protected): return True
    ...
    if getattr(vc, 'channel', None) and (not interaction.user.voice or interaction.user.voice.channel != vc.channel):
        raise discord.app_commands.AppCommandError("You must be in the active voice channel.")
    return True
